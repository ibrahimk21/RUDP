...
def main() -> int:
    ...
    if args.verify:
        ...
        value = bytes(dump[0]["value"])
        tcp_base, dropped, sequence, payload, timestamp = struct.unpack("=IIIIQ", value)
    ...
    run(["clang", "-O2", "-g", "-target", "bpf", f"-D__TARGET_ARCH_{architecture}", f"-DTARGET_PORT={args.port}", f"-DTARGET_RECORD={args.record}", f"-DTARGET_TCP={int(args.protocol == 'tcp')}", "-c", str(source), "-o", str(obj)])
